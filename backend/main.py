...
PARQET_PERFORMANCE_URL = "https://connect.parqet.com/performance"

# Abgeltungssteuer inkl. Soli — für Dividenden, bei denen Parqet keine Steuern kennt
KAPITALERTRAGSTEUER = 0.26375

# TODO: durch deine echten vier portfolioIds aus der Parqet-Console ersetzen
PARQET_PORTFOLIO_IDS = [
    "67c42af7ccb8e06ba97b0e60",
    "67c44927d5456f84f062c159",
    "67c57cd1e029c49c74d3d8d2",
    "67c58c211593c98763bad44f",
]
...
def get_valid_token():
    with Session(engine) as session:
        ...
        # SQLite speichert keine Zeitzone → Wert kommt naiv zurück, ist aber UTC.
        # Vor dem Vergleich das UTC-Etikett wieder ankleben.
        expires_at = parqet_token.expires_at
...
@app.get("/holdings/raw")
def get_holdings_raw():
    token = get_valid_token()
    response = httpx.post(
        PARQET_PERFORMANCE_URL,
        headers={"Authorization": f"Bearer {token}"},
        json={
            "portfolioIds": PARQET_PORTFOLIO_IDS,
...
@app.get("/holdings")
def get_holdings():
    token = get_valid_token()   # Türsteher: liefert garantiert gültigen Token

    response = httpx.post(
        PARQET_PERFORMANCE_URL,
        headers={"Authorization": f"Bearer {token}"},  # Token = Ausweis, bei jeder Anfrage vorzeigen
        json={                                         # json= → echtes JSON (NICHT data= wie beim Token-Endpoint!)
            "portfolioIds": PARQET_PORTFOLIO_IDS,
            "interval": {"type": "relative", "value": "1y"},
            "currency": "EUR",
            "includeCharts": False,
    ...
    for holding in data["holdings"]:
        # Filter 1+2: verkauft oder Cash → überspringen
        if holding["position"]["isSold"] or holding["asset"]["type"] == "cash":
            continue

        # --- korrigiertes Dividenden-Netto ---
        dividends = holding["performance"]["dividends"]["inInterval"]
        if dividends["taxes"] > 0:
            dividend_net = dividends["gainNet"]   # Steuern bekannt → netto direkt nehmen
        else:
            dividend_net = dividends["gainGross"] * (1 - KAPITALERTRAGSTEUER)  # sonst pauschal abziehen

        isin = holding["asset"]["isin"]
        if isin in aggregated:
            # ISIN schon gesehen (z.B. Amazon in zwei Depots) → aufaddieren
    ...
    # FIRE-Zahl: Summe aller Dividenden-Nettos (1 Jahr) ÷ 12
    annual_dividend_net = sum(position["dividend_net"] for position in aggregated.values())

    return {
        "monthly_dividend_net": annual_dividend_net / 12,
